...
def main():
 ...
 for c in cases:
  try:
   observation=observe_episode(source/(c['id']+'.wav'),c['plan']['event'],c['continuation'],ev,al,d);plan=reconstruct_episode(c['plan']['initial_state'],c['plan']['event'],c['continuation'],observation);repeat=reconstruct_episode(c['plan']['initial_state'],c['plan']['event'],c['continuation'],observation);row={'id':c['id'],'observation':observation,'plan':plan,'replay_identical':digest(plan)==digest(repeat),'pass':digest(plan)==digest(repeat)}
