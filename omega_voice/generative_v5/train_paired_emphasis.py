...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('root');ap.add_argument('--intrinsic',action='store_true');a=ap.parse_args();r=pathlib.Path(a.root);d=r/'continuation'/('intrinsic_emphasis_evaluator'if a.intrinsic else'paired_emphasis_evaluator');old=r/'continuation/prosodic_evaluator';pre=json.loads((d/'PREDECLARED.json').read_text());fresh=json.loads((d/'FRESH_FEATURES.json').read_text());complete=json.loads((d/'PREPARATION_COMPLETE.json').read_text());prior=json.loads((old/'FEATURES.json').read_text());indices=pre.get('feature_indices',list(range(len(NAMES))))
 ...
 for (group,voice,fold),rows in groups.items():
  for before,after in itertools.permutations(rows,2):
   ...
   x=(np.asarray(after['features'])-np.asarray(before['features']))[:,indices];y=np.array([int(i in after['gold'])-int(i in before['gold'])for i in range(len(x))]);pairs.append({'before':before['case']['id'],'after':after['case']['id'],'text_group':group,'voice':voice,'fold':fold,'features':x,'labels':y,'words':[w['word']for w in after['word_features']]})
 ...
 zero=int(model.predict(np.zeros((1,len(indices))))[0]);checks={'fresh_clocks_all_admitted':all(x['admitted']for x in fresh),'added_precision':all(folds[f]['added']['precision']>=.8 for f in ['new_text','new_text_and_voice']),'added_recall':all(folds[f]['added']['recall']>=.7 for f in ['new_text','new_text_and_voice']),'removed_precision':all(folds[f]['removed']['precision']>=.8 for f in ['new_text','new_text_and_voice']),'removed_recall':all(folds[f]['removed']['recall']>=.7 for f in ['new_text','new_text_and_voice']),'zero_difference_unchanged':zero==0}
 manifest={'model_sha256':sha(path),'feature_provenance':provenance(),'feature_indices':indices,'training_source_sha256':sha(__file__),'source_hashes':{'previous_features':sha(old/'FEATURES.json'),'fresh_features':sha(d/'FRESH_FEATURES.json'),'predeclared':sha(d/'PREDECLARED.json')},'classes':model.classes_.tolist(),'training_words':len(y),'training_pairs':len(training),'dependencies':{x:importlib.metadata.version(x)for x in ['scikit-learn','numpy','scipy','joblib','cmudict','praat-parselmouth']},'model_parameters':model.get_params(),'scope':'within-word paired acoustic changes without gap features'if a.intrinsic else'paired acoustic changes; no Mari data, state or word identity in fitting'};durable_json(d/'MODEL_PROVENANCE.json',manifest)
