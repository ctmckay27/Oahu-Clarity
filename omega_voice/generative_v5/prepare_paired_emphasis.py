...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('root');a=ap.parse_args();r=pathlib.Path(a.root);d=r/'continuation/paired_emphasis_evaluator';d.mkdir(exist_ok=True);old=r/'continuation/prosodic_evaluator';prior=json.loads((old/'FEATURES.json').read_text());old_groups={x['case']['text_group_sha256']for x in prior};meta=pathlib.Path('/tmp/mari-emphassess-data/gold_df.json');raw=[json.loads(x)for x in meta.read_text().splitlines()];groups=collections.defaultdict(list)
