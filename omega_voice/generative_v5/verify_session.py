...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('root');ap.add_argument('--mode',choices=['native','physical'],default='native');ap.add_argument('--output',default='session_continuity');ap.add_argument('--thought-recovery',action='store_true');a=ap.parse_args();r=pathlib.Path(a.root)
 ...
 def session(name):return PerformanceSession(r,d/name,ev,bank,sha(bank),mode=a.mode,aligner=aligner,temporal_policy='bounded_thought_recovery_v1' if a.thought_recovery else None)
