...
def main():
 ...
 for case in cases:
  i=case['source'];p=case['plan'];source=pathlib.Path(case['source_path']);out=d/f'{i}_duration.wav';receipt=realize(source,out,p,cal);q=ev.evaluate(out,case['text']);alignment=al.align(ev.load(out),case['text'],sha(out),q['wer']==0)
  ...
  combined=d/f'{i}_combined.wav';new_plan=compile_scene(case['text'],p['scene'],p['initial_state'],timeline=timeline_from_evaluation(dict(q,alignment=alignment)),policy='contrast_focus_v7');cr=contour(out,combined,new_plan);cq=ev.evaluate(combined,case['text']);ca=al.align(ev.load(combined),case['text'],sha(combined),cq['wer']==0)
  rows.append({'source':i,'variant':'duration_plus_contour_pressure','text':case['text'],'focus':case['focus'],'audio':str(combined),'receipt':cr,'quality':cq,'alignment':ca,'clock_error_s':max(abs(x[k]-y[k])for x,y in zip(alignment['words'],ca['words'])for k in ['start','end'])});durable_json(d/'RESULTS.json',rows);print(i,'combined',cq['wer'],cq['speaker_similarity'],flush=True)
 durable_json(d/'ASSESSMENT.json',{'quality_and_clock_pass':all(x['quality']['wer']==0 and x['quality']['identity_pass'] and x['clock_error_s']<=.08 for x in rows),'emphasis_and_UTMOS_pending':True,'full_completion':False})
