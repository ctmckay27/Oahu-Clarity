...
class ExplicitSceneCompiler:
 def __init__(self,nlp,policy='bounded_thought_recovery_v1'):
  self.nlp=nlp
  if policy not in {'bounded_thought_recovery_v1','epistemic_focus_v2','embodied_continuity_v3','linguistic_scope_v4','respiratory_budget_v5','listener_causal_v6','contrast_focus_v7'}:raise ValueError('unknown scene temporal policy')
 ...
 def compile(self,scene,text,prior=None,context=None):
  ...
  if assertion:
   if self.temporal_policy not in {'epistemic_focus_v2','embodied_continuity_v3','linguistic_scope_v4','respiratory_budget_v5','listener_causal_v6','contrast_focus_v7'}:raise ValueError('claim context requires epistemic_focus_v2 or successor')
  ...
  for at,source in strata(scene):
   ...
   for sent in doc.sents:
    ...
    def is_mari(token):
     ...
     return False
    def is_listener(token):
     subject=governor_subject(token)
     return subject is not None and (subject.lower_=='listener' or (subject.lower_=='they' and before_principal=='listener'))
    def mark(token):
     used.add(token.i)
    ...
    for token in tokens:
     ...
     if self.temporal_policy in {'listener_causal_v6','contrast_focus_v7'} and is_listener(token):
      ...
      if dimension:
       if nonasserted(token):unresolved.append({'at_word':at,'source':sentence,'reason':'listener observation is conditional or reported'})
       else:add(token,'listener_condition.'+dimension,0. if negative(token) else .75)
      continue
     ...
     if neg:
      if self.temporal_policy in {'listener_causal_v6','contrast_focus_v7'} and lemma=='care' and 'listener' in subtree:
       add(token,'relationship.concern',0.);continue
      if lemma=='remember':
     ...
     elif self.temporal_policy in {'listener_causal_v6','contrast_focus_v7'} and (lemma=='care' or token.lower_=='concerned') and 'listener' in subtree:add(token,'relationship.concern',.8)
     elif lemma=='feel':
      for child in token.children:
    ...
    if root_subj is not None:
     ...
     elif self.temporal_policy in {'listener_causal_v6','contrast_focus_v7'} and root_subj.lower_=='listener':principal='listener'
     elif self.temporal_policy in {'listener_causal_v6','contrast_focus_v7'} and root_subj.lower_=='they' and before_principal=='listener':principal='listener'
