...
def main():
 ...
 if a.stage=='align':
  ...
  for case in cases:
   ...
   # The fixed published human transcript is an independent source. These
   # controls are not synthesized candidates forced to a requested transcript.
   ...
   for i,token in enumerate(case['src_sentence']):
    ...
    offset+=n
   rows.append({'id':case['id'],'path':str(path),'source_case':case,'gold_indices':gold,'alignment':alignment,'transcript_authority':'fixed published human annotation, facebookresearch/emphassess pinned test data'});durable_json(d/'ALIGNED_CONTROLS.json',rows);print(case['id'],alignment['minimum_word_probability'],flush=True)
  durable_json(d/'PREDECLARED.json',{'controls':len(cases),'word_overlap_threshold':.5,'gates':{'precision_min':.8,'recall_min':.7,'non_speech_positive_frame_fraction_max':.01},'source_pipeline_documented_baseline':{'precision':1,'recall':.71,'F1':.83},'adaptation':'qualified CTC word clocks on same-language published human transcripts; no SimAlign/translation/WhisperX inference','scope':'acoustic word emphasis; no intent, Mari, naturalness or character judgment'});return
