...
class ConversionProbe:
 def __init__(self,root):
  ...
  def load(module,state,label,allowed_missing=(),allowed_extra=()):
   if label=='reference_encoder':
    # CAMPPlus's override performs these two compatibility renames but drops
    # the return value. Apply the exact remap and retain the actual load audit.
    state={k.replace('xvector.stats','stats',1) if k.startswith('xvector.stats') else k.replace('xvector.dense','dense',1) if k.startswith('xvector.dense') else k:v for k,v in state.items()}
   miss,extra=torch.nn.Module.load_state_dict(module,state,strict=False)
   if set(extra)-set(allowed_extra) or any(not any(k.startswith(p)for p in allowed_missing)for k in miss):raise ValueError('unloaded conversion weights '+label+repr((miss,extra)))
   loaded.append({'part':label,'missing':list(miss),'unexpected':list(extra),'allowed_missing_prefixes':list(allowed_missing),'allowed_unused_checkpoint_keys':list(allowed_extra)})
  ...
  # Pinned DiT.forward never defines or calls these three historical tensors.
  # Upstream ignores them via strict=False. All active weights remain required.
  load(model.cfm,strip(ck['cfm']),'cfm',allowed_extra=('estimator.content_mask_embedder.weight','estimator.skip_linear.weight','estimator.skip_linear.bias'))
  load(model.cfm_length_regulator,strip(ck['length_regulator']),'length_regulator')
 ...
 def render(self,source,destination,seed=93800):
  ...
  p.parent.mkdir(parents=True,exist_ok=True);native=p.with_suffix('.native22k.wav')
  if native.exists():raise FileExistsError(native)
  sf.write(native,y,sr,subtype='PCM_16')
  # Keep the original decoder artifact and make the existing 24k contract
