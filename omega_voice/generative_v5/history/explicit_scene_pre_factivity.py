...
class ExplicitSceneCompiler:
 def __init__(self,nlp,policy='bounded_thought_recovery_v1'):
  self.nlp=nlp
  if policy not in {'bounded_thought_recovery_v1','epistemic_focus_v2','embodied_continuity_v3','linguistic_scope_v4','respiratory_budget_v5','listener_causal_v6'}:raise ValueError('unknown scene temporal policy')
 ...
 def compile(self,scene,text,prior=None,context=None):
  ...
  if assertion:
   if self.temporal_policy not in {'epistemic_focus_v2','embodied_continuity_v3','linguistic_scope_v4','respiratory_budget_v5','listener_causal_v6'}:raise ValueError('claim context requires epistemic_focus_v2 or successor')
  ...
  for at,source in strata(scene):
   ...
   for sent in doc.sents:
    ...
    for token in tokens:
     ...
     if self.temporal_policy=='listener_causal_v6' and is_listener(token):
     ...
     if neg:
      if self.temporal_policy=='listener_causal_v6' and lemma=='care' and 'listener' in subtree:
     ...
     elif self.temporal_policy=='listener_causal_v6' and (lemma=='care' or token.lower_=='concerned') and 'listener' in subtree:add(token,'relationship.concern',.8)
     elif lemma=='feel':
    ...
    if root_subj is not None:
     ...
     elif self.temporal_policy=='listener_causal_v6' and root_subj.lower_=='listener':principal='listener'
     elif self.temporal_policy=='listener_causal_v6' and root_subj.lower_=='they' and before_principal=='listener':principal='listener'
