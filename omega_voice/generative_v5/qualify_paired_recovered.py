...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('root');a=ap.parse_args();r=pathlib.Path(a.root);d=r/'continuation/paired_emphasis_evaluator';ad=r/'continuation/alignment_padding';original=json.loads((d/'FRESH_FEATURES.json').read_text());manifest=json.loads((d/'MODEL_PROVENANCE.json').read_text());pre=json.loads((d/'PREDECLARED.json').read_text())
 ...
 for (group,voice,fold),group_rows in groups.items():
  for before,after in itertools.permutations(group_rows,2):
   ...
   matrix=np.asarray(after['features'])-np.asarray(before['features']);truth=[int(i in after['gold'])-int(i in before['gold'])for i in range(len(matrix))];pred=model.predict(matrix).tolist();results.append({'before':before['case']['id'],'after':after['case']['id'],'voice':voice,'fold':fold,'words':words,'truth':truth,'predicted':pred,'probabilities':model.predict_proba(matrix).tolist()})
 ...
 zero=int(model.predict(np.zeros((1,len(NAMES))))[0]);checks={'all88fresh_cases_included':len(rows)==len(pre['fresh_cases'])==88,'all_reference_clocks_admitted':all(x['admitted']for x in rows),'added_precision':all(folds[f]['added']['precision']>=.8 for f in folds),'added_recall':all(folds[f]['added']['recall']>=.7 for f in folds),'removed_precision':all(folds[f]['removed']['precision']>=.8 for f in folds),'removed_recall':all(folds[f]['removed']['recall']>=.7 for f in folds),'zero_difference_unchanged':zero==0}
