...
def main():
 ...
 checks={'all_valid':len(valid)==len(rows),'published_bins_monotonic':bool(np.all(np.diff(ordered)>0)),'causeless_gaps_not_rewarded':'mechanical_gaps' in valid and valid['mechanical_gaps']['score_nature']<valid['carrier']['score_nature'],'rejected_acting_not_rewarded':valid['retained_rejected']['score_nature']<valid['carrier']['score_nature']}
