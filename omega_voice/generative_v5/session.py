...
class PerformanceSession:
 def __init__(self,root,directory,evaluator,bank,bank_sha256):
  self.root=pathlib.Path(root);self.directory=pathlib.Path(directory);self.directory.mkdir(parents=True,exist_ok=True)
  self.evaluator=evaluator;self.bank_path=pathlib.Path(bank)
  if sha(self.bank_path)!=bank_sha256:raise ValueError('unselected control-bank bytes')
  self.bank=np.load(self.bank_path)['directions'];self.state_path=self.directory/'session_state.json'
 ...
 def _render_turn(self,request_id,text,scene=None,seed=88000,diagnostic=False):
  ...
  request_hash=digest({'text':text,'scene':scene or {},'seed':seed,'parent':digest(prior) if prior else None})
